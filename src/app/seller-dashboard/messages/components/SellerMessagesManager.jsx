...
const DEMO_CONVERSATIONS = [
  { id: 'conv-1', contact: 'Kolade Adeyemi', email: 'kolade@example.com', avatar: 'K', lastMessage: 'Bonjour, est-ce que vous avez ce produit en stock ?', lastTime: '10:32', unread: 2,
    messages: [
  ...
  { id: 'conv-2', contact: 'Fatou Diallo', email: 'fatou@example.com', avatar: 'F', lastMessage: 'Merci pour la confirmation.', lastTime: 'Hier', unread: 0,
    messages: [
      { id: 1, from: 'seller', text: 'Bonjour, votre commande CMD-0002 a bien été confirmée.', time: '14:00', date: 'Hier' },
      { id: 2, from: 'buyer', text: 'Merci pour la confirmation.', time: '14:45', date: 'Hier' },
    ] },
];
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-4 border-b border-border flex items-center gap-3">
        ...
        <a href={`mailto:${conv.email}`} className="p-2 border border-border rounded-lg hover:bg-muted transition-smooth text-muted-foreground" title={t('common.email')}>
          ...
          return (
            <div key={msg.id}>
              ...
              <div className={`flex ${msg.from === 'seller' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-sm shadow-subtle ${msg.from === 'seller' ? 'bg-primary text-primary-foreground rounded-br-sm' : 'bg-card border border-border text-foreground rounded-bl-sm'}`}>
                  <p>{msg.text}</p>
                  <p className={`text-[10px] mt-1 ${msg.from === 'seller' ? 'text-primary-foreground/70 text-right' : 'text-muted-foreground'}`}>{msg.time}</p>
                </div>
              </div>
            </div>
          );
...
export default function SellerMessagesManager() {
  ...
  const persist = (list) => { setConversations(list); localStorage.setItem('bimark_seller_messages', JSON.stringify(list)); };
  ...
  const sendMessage = (text) => {
    ...
    const msg = { id: Date.now(), from: 'seller', text, time: new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }), date: t('dashboard.today') };
  ...
  const filtered = conversations.filter(c => c.contact.toLowerCase().includes(search.toLowerCase()) || c.lastMessage.toLowerCase().includes(search.toLowerCase()));
